...
2. 不同特权级有什么区别？
   ...
   决定了程序能访问的程序段/代码段的不同。内核指令只有Ring0可以执行。
...
1. 一条指令在执行时会有哪些可能的特权级判断？
   ...
   判断CPL、DPL，取其权限小的，如果仍大于门权限则通过判断。
...
3. int指令在ring0和ring3的执行行为有什么不同？
   ...
   在ring3执行时多了一个SS:ESP的压栈。


4. 如何利用int和iret指令完成不同特权级的切换？

   构造一个栈，压入不同的SS:ESP，使得还原时换到不同的特权级。
...
1. 一条指令执行时最多会出现多少次地址转换？
   ...
   取决于页表有多少级。一般来说段转换一次，页转换两次（页表目录，页表）
